...
$+108.2\%$ improvement over Content-KNN (0.4346), combining Cleora behavioral
...
Cleora's 375k behavioral index, while DIF-SASRec covers items outside that graph.
...
are served from LRU caches at under 1\,ms, leaving Tantivy BM25
...
The AgentPool of 8 DIF-SASRec instances sustains 10+ concurrent users
with no weight corruption (validated at 20 concurrent requests).
Each agent holds a pretrained snapshot in CPU RAM ($\sim$148\,MB);
fine-tuned weights occupy GPU memory only for the request duration.
Total pool footprint is 1.18\,GB, leaving headroom for FAISS indices
and encoder models.
